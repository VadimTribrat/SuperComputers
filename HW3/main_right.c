...
int main()
{
    ...
    //     w_k1 = w_k + residual;
    //     std::cout << sqrt(scalar_product(true_res - w_k1, true_res - w_k1, h1, h2)) << "\n";
    //     w_k = w_k1;
    // }
    //Matrix residual = operatorA(w_k, x, y) - right_part;
    //std::cout << residual << "\n";
...
Matrix operatorA(Matrix& w, double * x, double * y)
{
    ...
    {
        std::cout << ax[i][N] << "\n";
        bottom[i] = -2*pby[i][1]/h2 + q(x[i], y[0]) * w[i][0] - ax[i][0]; 
        top[i] = 2 * pby[i][N] / h2 + q(x[i], y[N]) * w[i][N] - ax[i][N];
    }

    for (size_t j = 1; j < w_copy.getColumns() - 1; ++j)
    {
        left[j] = -2 * pax[1][j] / h1 + (q(x[0], y[j]) + 2 / h1) * w[0][j] - by[0][j];
        right[j] = 2 * pax[M][j] / h1 + (q(x[M], y[j]) + 2 / h1) * w[M][j] - by[M][j];
    }
    ...
    a1b1 = -2 * pax[1][0] / h1 - 2 * pby[0][1] / h2 + (q(x[0], y[0]) + 2 / h1 + 2 / h2 * 0) * w[0][0];
    a2b1 = 2 * pax[M][0] / h1 - 2 * pby[M][1] / h2 + (q(x[M], y[0]) + 2 / h1 + 2 / h2 * 0) * w[M][0];
    a2b2 = 2 * pax[M][N] / h1 + 2 * pby[M][N] / h2 + (q(x[M], y[N]) + 2 / h1 + 2 / h2 * 0) * w[M][N];
    a1b2 = -2 * pax[1][N] / h1 + 2 * pby[0][N] / h2 + (q(x[0], y[N]) + 2 / h1 + 2 / h2 * 0) * w[0][N];

    for (size_t i = 0; i < w_copy.getRows(); ++i)
    {
        for (size_t j = 0; j < w_copy.getColumns(); ++j)
        {
...
Matrix right(Matrix& w, double * x, double * y)
{
    ...
    {
        // std::cout << "psiR: " << x[0] << " " << y[j] << " F: " <<
        //     F(x[M], y[j]) << " psiL " << 2 * psiR(x[M], y[j]) / h1 << "\n";
