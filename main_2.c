...
int main()
{
    ...
    Matrix w(gridX - 1, gridY), r(gridX - 1, gridY);
    Matrix true_res(gridX + 1, gridY + 1);
    Matrix w_k(gridX -1, gridY), w_k1(gridX - 1, gridY);
...
Matrix operatorA(Matrix& w, double * x, double * y)
{
    ...
    Matrix a(M + 1, N + 1), b(M + 1, N + 1);
    auto [ax, by] = delta_w(w, x, y);
    Matrix mat1(M + 1, N + 1), mat2(M + 1, N + 1), F_mat(M + 1, N + 1), w_copy(w);
    std::vector<double> bottom(w.getRows(), 0), right(w.getColumns(), 0)
    , top(w.getRows(), 0), left(w.getColumns(), 0);
    ...
    {
        bottom[i] = -2*by[i][1]/h2 + (q(x[i + 1], y[0]) + 2/h2) * w[i][0] - ax[i][0]; 
    }
    for (size_t i = 1; i < M; ++i)
    {
        top[i] = -ax[i][N] - (by[i][N - 1] - b[i][N] * w[i][N] / h2) / h2 + q(x[i + 1], y[N]) * w[i][N];
    }
    for (size_t j = 1; j < N; ++j)
    {
        right[j] = -by[M][j] - (ax[M-1][j] - a[M][j] * w[M][j] / h1) / h1 + q(x[M + 1], y[j]) * w[M][j];
    }
    for (size_t j = 1; j < N; ++j)
    {
        left[j] = -by[0][j] - (ax[1][j] - a[0][j] * w[0][j] / h1) / h1 + q(x[1], y[j]) * w[0][j];
    }
    
     a1b1 = -(ax[0][1]) / h1;
     a2b1 = 2 * ax[M][0]/h1 - 2 * by[M][1]/h2 + (q(x[M], y[0]) + 2/h1 + 2/h2)*w[M][0];
     a2b2 = (ax[M][N] + a[M+1][N]*w[M][N])/h1/h1 -
         (by[M][1] - b[M][1]*w[M][0])/h2;
     a1b2 = -(ax[1][N] - a[0][N]*w[0][N]/h1)/h1 + 
         (by[0][N] + b[0][N]*w[0][N]/h2)/h2 + q(x[1], y[N])*w[0][N];
...
Matrix right(Matrix& w, double * x, double * y)
{
    ...
    for (size_t i = 1; i < M; ++i)
    {
        right_mat[i][0] = F(x[i + 1], y[0]) + 2 * psi(x[i + 1], y[0]) / h2;
        right_mat[i][N] = F(x[i + 1], y[N]) + b[i][N] * true_func(x[i + 1], y[N + 1]) / (h2 * h2);
    }
    for (size_t j = 1; j < N; ++j)
    {
        right_mat[0][j] = F(x[1], y[j]) + a[0][j] * true_func(x[0], y[j]) / (h1 * h1);
        right_mat[M][j] = F(x[M], y[j]) + a[M][j] * true_func(x[M + 1], y[j]) / (h2 * h2);
    }
